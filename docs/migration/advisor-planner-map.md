...
`advisor-planner` section to its `skill-team/v3` successor. **`skills/advisor-planner/` is not
removed by this migration** — it remains available for ad hoc, single-session advisor/planner usage
...
`skills/advisor-planner/` remains in the repository, unmodified by this migration, for users who want
a single-session advisor/planner workflow instead of the full staged pipeline. This document only
records where its responsibilities live in `skill-team/v3` for anyone migrating a project from one
mode to the other.
