...
`docs/ai/<slug>/PROGRESS.md` first. Do not duplicate queues or status into `AGENTS.md`. The portable contract is `skill-team/v3`; `next_skill` and every live v2 alias are invalid.
...
It must use the canonical `discovery/`, `plan/`, `routing/`, `execution/`, `review/`, `release/`, `blockers/`, `findings/`, and `handoffs/` paths below that workflow root. A ready handoff is immutable for its revision.

## Core boundary

This guide does not change Skill Team behavior. Clarification, compact `plan/SPEC.md`, requirement checklists, and consistency analysis remain planning substeps; review only classifies and returns findings; release relies on recorded evidence. Use the core migration dry run and hash-safe rollback, not Codex-specific state.
