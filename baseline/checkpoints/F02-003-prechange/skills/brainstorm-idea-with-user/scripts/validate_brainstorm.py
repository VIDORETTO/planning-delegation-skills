...
ALLOWED_STATUSES = READY | {IN_PROGRESS, "DISCOVERY_BLOCKED"}
...
REQUIRED_BRAINSTORM_HEADINGS = (
    ...
    "Questões em aberto", "Registros de esclarecimento", "Riscos", "Sugestões recusadas",
...
def validate_clarification_records(text: str, path: Path) -> list[str]:
    """Reject repeated or conflicting active discovery decisions."""
    records = section(text, "Registros de esclarecimento")
    if not records:
        return [f"missing clarification records section: {path}"]
    ...
    for index, match in enumerate(matches):
        ...
        if question_id in record_ids:
            errors.append(f"duplicate clarification question ID: {question_id}")
        ...
        values = {label: clarification_value(block, label) for label in (
            "Pergunta", "Chave de decisão", "Escopo", "Estado", "Feita por", "Resposta", "Fonte",
            "Autoridade", "Revisão", "Recomendação", "Aceita", "Substitui", "Prontidão após resposta",
        )}
        for label, value in values.items():
            if not value:
                errors.append(f"clarification record missing {label}: {question_id}")
        if values["Escopo"] != "DISCOVERY":
            errors.append(f"clarification record has invalid discovery scope: {question_id}")
        status = values["Estado"]
        if status not in {"PENDING", "ANSWERED", "SUPERSEDED"}:
            errors.append(f"clarification record has invalid status: {question_id}")
        if values["Feita por"] != SKILL:
            errors.append(f"clarification record has invalid owner: {question_id}")
        if values["Aceita"] not in {"true", "false"}:
            errors.append(f"clarification record has invalid acceptance state: {question_id}")
        if values["Prontidão após resposta"] not in {"READY", "NOT_READY"}:
            errors.append(f"clarification record has invalid readiness result: {question_id}")
        if not re.fullmatch(r"(?:SRC|DEC|CR|ASM|BR)-\d{3}", values["Fonte"]):
            errors.append(f"clarification record source lacks stable ID: {question_id}")
        if not values["Revisão"].isdigit() or int(values["Revisão"] or "0") < 1:
            errors.append(f"clarification record has invalid revision: {question_id}")
        if status == "PENDING" and (values["Resposta"] != "NONE" or values["Aceita"] != "false"):
            errors.append(f"pending clarification cannot contain an accepted answer: {question_id}")
        if status == "PENDING":
            pending_records += 1
        if status == "ANSWERED" and (values["Resposta"] == "NONE" or values["Aceita"] != "true"):
            errors.append(f"answered clarification lacks an accepted answer: {question_id}")
        if status == "SUPERSEDED" and not re.fullmatch(r"Q-\d{3}", values["Substitui"]):
            errors.append(f"superseded clarification must identify its replacement: {question_id}")
        normalized_question = re.sub(r"\W+", " ", values["Pergunta"].lower()).strip()
        if normalized_question in questions:
            errors.append(f"duplicate clarification question: {question_id} duplicates {questions[normalized_question]}")
        else:
            questions[normalized_question] = question_id
        if status == "ANSWERED":
            key = values["Chave de decisão"].lower()
            answer = values["Resposta"].lower()
            if key in decisions and decisions[key][0] != answer:
                errors.append(f"contradictory active clarification decisions: {question_id} conflicts with {decisions[key][1]}")
            else:
                decisions[key] = (answer, question_id)
    if pending_records > 1:
        errors.append("more than one clarification question is pending")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if progress:
        ...
        expected_artifacts = {f"docs/ai/{slug}/discovery/brainstorm/BRAINSTORM.md"}
    ...
    if brainstorm:
        ...
        for error in validate_clarification_records(brainstorm_body, paths["brainstorm"]):
            report.error(error)
    ...
    revisions = [
        progress.get("discovery_revision"),
        brainstorm.get("discovery_revision"),
    ]
    if handoff:
        revisions.append(
            handoff.get("input_revision")
        )
