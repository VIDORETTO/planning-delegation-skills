...
Explicit user decisions remain authoritative for product intent. Record any conflict with applicable
law or stronger security controls for escalation; brainstorm must not silently resolve or override it.
...
Ask exactly one material question per interaction. Do not repeat an answered or superseded question.
Explain in one sentence why its answer changes acceptance, architecture, risk, or release. Prefer two
to five mutually exclusive options when useful, state a recommendation as a `PROPOSAL`, and accept a
custom answer. Update the clarification record and derived decision immediately after each answer.

Use a stable `Q-*` ID for every clarification in `BRAINSTORM.md`. Each record must retain the question,
decision key, scope, status, answer, source, authority, revision, recommendation, acceptance state, and
readiness result. A recommendation is never a decision until its acceptance is recorded. Reuse neither a
question ID nor a decision key for conflicting active answers; supersede the earlier record instead.

After every accepted answer, recompute the readiness checklist and record whether discovery is `READY`,
`NOT_READY`, or requires a return. After five accepted questions, summarize remaining gaps and ask whether
to continue; this is a checkpoint, not a completeness limit. Continue when a structural gap prevents safe
planning and the user agrees.
...
If an answer changes the audience, problem, intended outcome, or MVP, keep the answer and its authority,
mark the affected planning record `RETURNED_TO_DISCOVERY`, and remain or return to this discovery owner.
Do not let planning reinterpret, overwrite, or discard a confirmed user decision.
