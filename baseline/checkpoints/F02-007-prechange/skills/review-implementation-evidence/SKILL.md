...
starting. A stale handoff is not reviewable; record the stale implementation input and return control
to `execute-routed-task` without approving or generating an outgoing review handoff.
...
### 4. Classify and trace every finding

Use one severity exactly: `BLOCKING`, `HIGH`, `MEDIUM`, `LOW`, `QUESTION`, or `OUT_OF_SCOPE`, and
one gap type exactly: `missing`, `partial`, `contradicts`, or `unrequested`. For every actionable
finding, record the requirement ID(s), task ID, return owner, and an existing relative evidence path
to the actual code, command output, or reviewed artifact. Compare that evidence against the
requirement, plan decision, task, acceptance criteria, and applicable governance obligation. See
...
| Return to discovery | Audience, problem, outcome, or MVP intent changed | `status: BRAINSTORM_IN_PROGRESS`, `required_skill: brainstorm-idea-with-user` |

Never implement the fix yourself, even a "small" one. Record it as a finding and route it back.

### 6. Produce artifacts and stop

Write `review/REVIEW-REPORT.md` and one `findings/<FINDING-ID>.md` per finding. Generate either
`handoffs/REVIEW-TO-IMPLEMENTATION.md` (bounded changes) or `handoffs/REVIEW-TO-RELEASE.md`
(approved) — never both for the same revision. Routing, planning, investigation, and brainstorm
returns use the finding plus `PROGRESS.md`; do not create an ad hoc review return handoff. A clean
approval has no finding files and states `No findings` in the report rather than creating an empty
finding.
...
The reviewer records severity, gap type, requirement/task traceability, actual evidence, and return
owner, but does not silently assume ownership of the return stage. It names the required skill and
stops. Never edit source, execution evidence, plan tasks, routing assignments, or acceptance criteria
while reviewing.
