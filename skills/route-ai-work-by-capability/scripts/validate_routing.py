...
Legacy usage remains supported:
    python validate_routing.py <tasks-directory> <routing-document> --tiers A,B
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("root", type=Path, help="Workflow root, or task directory in legacy mode")
    parser.add_argument("routing_document", nargs="?", type=Path, help="Legacy routing path")
    parser.add_argument("--tiers", help="Legacy comma-separated allowed model IDs")
...
def parse_routes(path: Path) -> tuple[list[Route], list[str]]:
    required = {"Task", "Executor", "Tier", "Hard gate", "Score", "Confidence", "Reviewer", "Review mode", "Batch", "Locks", "Rationale"}
    ...
    for row, line in records:
        ...
        routes.append(Route(task_id, row["Executor"], row["Tier"].upper(), row["Hard gate"], score, row["Confidence"].upper(), row["Reviewer"], row["Review mode"].upper(), row["Batch"], locks, row["Rationale"], line))
...
def validate_legacy(tasks_root: Path, routing_path: Path, allowed: set[str] | None) -> int:
    tasks = parse_tasks(tasks_root)
    routes, errors = parse_routes(routing_path)
    ...
    for task_id in sorted(task_map.keys() & route_map.keys()):
        task, route = task_map[task_id], route_map[task_id]
        if not task.executor: errors.append(f"task has no embedded Executor: {task_id}")
        elif task.executor != route.executor: errors.append(f"owner mismatch {task_id}: task={task.executor} routing={route.executor}")
        if allowed and route.executor not in allowed: errors.append(f"executor outside allowed tiers: {task_id} -> {route.executor}")
...
def main() -> int:
    ...
    if args.routing_document:
        allowed = {x.strip() for x in args.tiers.split(",") if x.strip()} if args.tiers else None
        return validate_legacy(root, args.routing_document.resolve(), allowed)
    if not root.is_dir():
